    ...
    searchBtn.addEventListener('click', () => {
      ...
      if (searchYear) {
        const filteredBooks = books.filter(book => {
          const birthYear = book.authors.length > 0 ? book.authors[0].birth_year : '';
...
function sortBooks(books) {
  books.sort((a, b) => {
    const birthYearA = a.authors.length > 0 ? a.authors[0].birth_year : Infinity;
    const birthYearB = b.authors.length > 0 ? b.authors[0].birth_year : Infinity;
...
function renderTable(books) {
  ...
  books.forEach(book => {
    ...
    const birthYear = book.authors.length > 0 ? book.authors[0].birth_year : 'N/A';
